    ...
    RayTracer.prototype.render = function (scene, ctx, screenWidth, screenHeight) {
        ...
        for (var y = 0; y < screenHeight; y++) {
            for (var x = 0; x < screenWidth; x++) {
                var color = this.traceRay({ start: scene.camera.pos,
...
function exec() {
    var width = 256;
    var height = 256;
    var canvas = document.querySelector("#ts-canvas");
    var info = document.querySelector("#ts-info");
    var button = document.querySelector("#ts-button");
    var i = 0;
    var rendersPerBatch = 1; // Change to run several raytraces on first click.
    var times = [];
